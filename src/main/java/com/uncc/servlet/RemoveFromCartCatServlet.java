 ...
 */
public class RemoveFromCartCatServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ResultSet ps2 = stmt.executeQuery("select c.accessoryname,c.price,c.accessory_id, a.order_id from accessorycat c\n" +
                    "INNER JOIN add_to_cart_accessory_cat a on a.accessory_id=c.accessory_id \n" +
                    "where a.order_id = (select max(order_id) from add_to_cart_accessory_cat) ;");
            ...
            // }
            for (String item:items) {
                ps3 = con.prepareStatement("delete from add_to_cart_accessory_cat where order_id=? and email=? and accessory_id=?");
                ...
                CallableStatement cStmt = con.prepareCall("{call update_accessorycat_remove(?)}");//call meetapet.update_accessorydog(5);
            ...
            response.sendRedirect("ThankYouCat.jsp");
        }
